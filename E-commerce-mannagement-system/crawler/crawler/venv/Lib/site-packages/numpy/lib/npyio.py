...
class BagObj(object):
    ...
    Examples
    --------
    >>> from numpy.analysis.npyio import BagObj as BO
...
class NpzFile(Mapping):
    ...
    >>> npz = np.load(outfile)
    >>> isinstance(npz, np.analysis.io.NpzFile)
    True
    >>> sorted(npz.files)
    ['x', 'y']
...
def load(file, mmap_mode=None, allow_pickle=False, fix_imports=True,
    ...
    analysis.format.open_memmap : Create or load a memory-mapped ``.npy`` file.
...
def save(file, arr, allow_pickle=True, fix_imports=True):
    ...
    Notes
    -----
    For a description of the ``.npy`` format, see :py:mod:`numpy.analysis.format`.
...
def savez(file, *args, **kwds):
    ...
    description of the ``.npy`` format, see :py:mod:`numpy.analysis.format`.
