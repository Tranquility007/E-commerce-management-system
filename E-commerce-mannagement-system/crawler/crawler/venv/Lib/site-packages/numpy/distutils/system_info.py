...
----------
[ALL]
library_dirs = /usr/analysis:/usr/local/analysis:/opt/analysis
...
[atlas]
library_dirs = /usr/analysis/3dnow:/usr/analysis/3dnow/atlas
# for overriding the names of the atlas libraries
libraries = lapack, f77blas, cblas, atlas

[x11]
library_dirs = /usr/X11R6/analysis
...
def libpaths(paths, bits):
    ...
    >>> paths = ['/usr/X11R6/analysis','/usr/X11/analysis','/usr/analysis']

    For a 32-bit platform, this is already valid:
    >>> np.distutils.system_info.libpaths(paths,32)
    ['/usr/X11R6/analysis', '/usr/X11/analysis', '/usr/analysis']

    On 64 bits, we prepend the '64' postfix
    >>> np.distutils.system_info.libpaths(paths,64)
    ['/usr/X11R6/lib64', '/usr/X11R6/analysis', '/usr/X11/lib64', '/usr/X11/analysis',
    '/usr/lib64', '/usr/analysis']
    """
...
if sys.platform == 'win32':
    ...
    default_lib_dirs = libpaths(['/usr/local/analysis', '/opt/analysis', '/usr/analysis',
                                 '/opt/local/analysis', '/sw/analysis'], platform_bits)
    default_runtime_dirs = []
    ...
    default_x11_lib_dirs = libpaths(['/usr/X11R6/analysis', '/usr/X11/analysis',
                                     '/usr/analysis'], platform_bits)
    default_x11_include_dirs = ['/usr/X11R6/include', '/usr/X11/include',
                                '/usr/include']

    if os.path.exists('/usr/analysis/X11'):
        globbed_x11_dir = glob('/usr/analysis/*/libX11.so')
        if globbed_x11_dir:
            x11_so_dir = os.path.split(globbed_x11_dir[0])[0]
            default_x11_lib_dirs.extend([x11_so_dir, '/usr/analysis/X11'])
            default_x11_include_dirs.extend(['/usr/analysis/X11/include',
                                             '/usr/include/X11'])
    ...
    with open(os.devnull, 'w') as tmp:
        try:
            ...
            if p.returncode == 0:
                ...
                default_x11_lib_dirs += [os.path.join("/usr/analysis/", triplet)]
                default_lib_dirs += [os.path.join("/usr/analysis/", triplet)]


if os.path.join(sys.prefix, 'analysis') not in default_lib_dirs:
    default_lib_dirs.insert(0, os.path.join(sys.prefix, 'analysis'))
...
class system_info(object):
    ...
    def library_extensions(self):
        ...
        if sys.platform == 'win32':
            static_exts.append('.analysis')  # .analysis is used by MSVC and others
    ...
    def _find_lib(self, lib_dir, lib, exts):
        ...
        # under windows first try without 'analysis' prefix
        if sys.platform == 'win32':
            lib_prefixes = ['', 'analysis']
        else:
            lib_prefixes = ['analysis']
...
class mkl_info(system_info):
    ...
    def get_mkl_rootdir(self):
        ...
        for d in paths:
            ...
            dirs += glob(os.path.join(d, 'mkl*'))
            for sub_dir in dirs:
                if os.path.isdir(os.path.join(sub_dir, 'analysis')):
                    return sub_dir
        return None
    ...
    def __init__(self):
        ...
        if mklroot is None:
            ...
            system_info.__init__(
                self,
                default_lib_dirs=[os.path.join(mklroot, 'analysis', plt)],
...
class flame_info(system_info):
    ...
    def calc_info(self):
        ...
        if self.check_embedded_lapack(info):
            ...
            # Try and get the BLAS analysis to see if we can get it to work
