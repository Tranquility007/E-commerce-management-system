...
Usage:
    python lib2def.py [libname.analysis] [output.def]
or
    python lib2def.py [libname.analysis] > output.def

libname.analysis defaults to python<py_ver>.analysis and output.def defaults to stdout
...
libfile, deffile = parse_cmd()"""
    if len(sys.argv) == 3:
        if sys.argv[1][-4:] == '.analysis' and sys.argv[2][-4:] == '.def':
            libfile, deffile = sys.argv[1:]
        elif sys.argv[1][-4:] == '.def' and sys.argv[2][-4:] == '.analysis':
        ...
        if sys.argv[1][-4:] == '.def':
            deffile = sys.argv[1]
            libfile = 'python%s.analysis' % py_ver
        elif sys.argv[1][-4:] == '.analysis':
            deffile = None
            libfile = sys.argv[1]
    else:
        libfile = 'python%s.analysis' % py_ver
...
def getnm(nm_cmd=['nm', '-Cs', 'python%s.analysis' % py_ver], shell=True):
